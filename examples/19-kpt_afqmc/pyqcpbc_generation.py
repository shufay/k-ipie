...
chol_ao = chol_ao.transpose(4, 1, 2, 0, 3).copy() # (nq, nk, nmo, nmo, nchol) -> (nchol, nk, nmo, nq, nmo).

savefilename = args.input.split(".")[0] + "_ao.h5"
with h5py.File(filepath + savefilename, 'w') as f:
    f['mo_coeff'] = mf.get_mo()
    f['mo_occ'] = mf.get_noccs()
    f['hcore'] = hcore.transpose(2, 0, 1) # nk, nmo, nmo
    f['chol'] = chol_ao / numpy.sqrt(nk)
    f['kpoints'] = kpts_frac
    f['e0'] = Enuc + Emadelung
