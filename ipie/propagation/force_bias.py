        ...
        if trial.nbeta > 0:
            ...
            for iq in range(len(hamiltonian.Sset)):
                ...
                for ik in range(hamiltonian.nk):
                    ikpq = hamiltonian.ikpq_mat[iq_real, ik]
                    vbias_plus[:, :, iq] += .5j * (
                            xp.einsum("igp, aip -> ag", trial._rchola[iq, ik], Ghalfa_reshape[:, ik, :, ikpq, :], optimize=True) + 
                            xp.einsum("igp, bip -> bg", trial._rcholb[iq, ik], Ghalfb_reshape[:, ik, :, ikpq, :], optimize=True))
                    vbias_plus[:, :, iq] += .5j * (
                            xp.einsum("pgi, aip -> ag", trial._rcholbara[iq, ik], Ghalfa_reshape[:, ikpq, :, ik, :], optimize=True) + 
                            xp.einsum("pgi, bip -> bg", trial._rcholbarb[iq, ik], Ghalfb_reshape[:, ikpq, :, ik, :], optimize=True))
        ...
        if trial.nbeta > 0:
            ...
            if config.get_option("use_gpu"):
                if len(hamiltonian.Sset) > 0:
                    iSset = xp.arange(len(hamiltonian.Sset))
                    ik_Sset = xp.repeat(
                                xp.arange(hamiltonian.nk), 
                                len(hamiltonian.Sset)).reshape(hamiltonian.nk, len(hamiltonian.Sset)).T
                    ikpq_S = hamiltonian.ikpq_mat[hamiltonian.Sset]
                    ...
                    tmp1 = xp.einsum("qkigp, qkaip -> agq", trial._rchola_chunk[iSset], Gak_kpq, optimize=True) + \
                           xp.einsum("qkigp, qkaip -> agq", trial._rcholb_chunk[iSset], Gbk_kpq, optimize=True)
                    tmp2 = xp.einsum("qkpgi, qkaip -> agq", trial._rcholbara_chunk[iSset], Gakpq_k, optimize=True) + \
                           xp.einsum("qkpgi, qkaip -> agq", trial._rcholbarb_chunk[iSset], Gbkpq_k, optimize=True)
                ...
                if len(hamiltonian.Qplus) > 0:
                    ...
                    ik_Qplus = xp.repeat(
                                xp.arange(hamiltonian.nk), 
                                len(hamiltonian.Qplus)).reshape(hamiltonian.nk, len(hamiltonian.Qplus)).T
                    ikpq_Q = hamiltonian.ikpq_mat[hamiltonian.Qplus]
                    ...
                    tmp1 = xp.einsum("qkigp, qkaip -> agq", trial._rchola_chunk[iQplus_real], Gak_kpq, optimize=True) + \
                           xp.einsum("qkigp, qkaip -> agq", trial._rcholb_chunk[iQplus_real], Gbk_kpq, optimize=True)
                    tmp2 = xp.einsum("qkpgi, qkaip -> agq", trial._rcholbara_chunk[iQplus_real], Gakpq_k, optimize=True) + \
                           xp.einsum("qkpgi, qkaip -> agq", trial._rcholbarb_chunk[iQplus_real], Gbkpq_k, optimize=True)
                ...
                for iq in range(len(hamiltonian.Sset)):
                    ...
                    for ik in range(hamiltonian.nk):
                        ikpq = hamiltonian.ikpq_mat[iq_real, ik]
                        vbias_batch_plus_send[:, chol_idxs_chunk, iq] += .5j * (
                                xp.einsum("igp, aip -> ag", trial._rchola_chunk[iq, ik], Ghalfa[:, ik, :, ikpq, :], optimize=True) + 
                                xp.einsum("igp, bip -> bg", trial._rcholb_chunk[iq, ik], Ghalfb[:, ik, :, ikpq, :], optimize=True))
                        vbias_batch_plus_send[:, chol_idxs_chunk, iq] += .5j * (
                                xp.einsum("pgi, aip -> ag", trial._rcholbara_chunk[iq, ik], Ghalfa[:, ikpq, :, ik, :], optimize=True) + 
                                xp.einsum("pgi, bip -> bg", trial._rcholbarb_chunk[iq, ik], Ghalfb[:, ikpq, :, ik, :], optimize=True))

                        vbias_batch_minus_send[:, chol_idxs_chunk, iq] += .5 * (
                                xp.einsum("igp, aip -> ag", trial._rchola_chunk[iq, ik], Ghalfa[:, ik, :, ikpq, :], optimize=True) + 
                                xp.einsum("igp, bip -> bg", trial._rcholb_chunk[iq, ik], Ghalfb[:, ik, :, ikpq, :], optimize=True))
                        vbias_batch_minus_send[:, chol_idxs_chunk, iq] -= .5 * (
                                xp.einsum("pgi, aip -> ag", trial._rcholbara_chunk[iq, ik], Ghalfa[:, ikpq, :, ik, :], optimize=True) + 
                ...
                for iq in range(len(hamiltonian.Sset), len(hamiltonian.Sset) + len(hamiltonian.Qplus)):
                    ...
                    for ik in range(hamiltonian.nk):
                        ikpq = hamiltonian.ikpq_mat[iq_real, ik]
                        vbias_batch_plus_send[:, chol_idxs_chunk, iq] += .5j * xp.sqrt(2) * (
                                xp.einsum("igp, aip -> ag", trial._rchola_chunk[iq, ik], Ghalfa[:, ik, :, ikpq, :], optimize=True) + 
                                xp.einsum("igp, bip -> bg", trial._rcholb_chunk[iq, ik], Ghalfb[:, ik, :, ikpq, :], optimize=True))
                        vbias_batch_plus_send[:, chol_idxs_chunk, iq] += .5j * xp.sqrt(2) * (
                                xp.einsum("pgi, aip -> ag", trial._rcholbara_chunk[iq, ik], Ghalfa[:, ikpq, :, ik, :], optimize=True) + 
                        ...
                        vbias_batch_minus_send[:, chol_idxs_chunk, iq] += .5 * xp.sqrt(2) * (
                                xp.einsum("igp, aip -> ag", trial._rchola_chunk[iq, ik], Ghalfa[:, ik, :, ikpq, :], optimize=True) + 
                                xp.einsum("igp, bip -> bg", trial._rcholb_chunk[iq, ik], Ghalfb[:, ik, :, ikpq, :], optimize=True))
                        vbias_batch_minus_send[:, chol_idxs_chunk, iq] -= .5 * xp.sqrt(2) * (
                                xp.einsum("pgi, aip -> ag", trial._rcholbara_chunk[iq, ik], Ghalfa[:, ikpq, :, ik, :], optimize=True) + 
                ...
                if len(hamiltonian.Qplus) > 0:
                    ...
                    for iq in range(len(hamiltonian.Sset)):
                        ...
                        for ik in range(hamiltonian.nk):
                            ikpq = hamiltonian.ikpq_mat[iq_real, ik]
                            vbias_batch_plus_send[:, chol_idxs_chunk, iq] += .5j * (
                                    xp.einsum("igp, aip -> ag", trial._rchola_chunk[iq, ik], Ghalfa_recv[:, ik, :, ikpq, :], optimize=True) + 
                                    xp.einsum("igp, bip -> bg", trial._rcholb_chunk[iq, ik], Ghalfb_recv[:, ik, :, ikpq, :], optimize=True))
                            vbias_batch_plus_send[:, chol_idxs_chunk, iq] += .5j * (
                                    xp.einsum("pgi, aip -> ag", trial._rcholbara_chunk[iq, ik], Ghalfa_recv[:, ikpq, :, ik, :], optimize=True) + 
                                    xp.einsum("pgi, bip -> bg", trial._rcholbarb_chunk[iq, ik], Ghalfb_recv[:, ikpq, :, ik, :], optimize=True))
            ...
            if config.get_option("use_gpu"):
                ...
                for iq in range(len(hamiltonian.Sset), len(hamiltonian.Sset) + len(hamiltonian.Qplus)):
                    ...
                    for ik in range(hamiltonian.nk):
                        ikpq = hamiltonian.ikpq_mat[iq_real, ik]
                        vbias_batch_plus_send[:, chol_idxs_chunk, iq] += .5j * (
                                xp.sqrt(2) * xp.einsum("igp, aip -> ag", trial._rchola_chunk[iq, ik], Ghalfa[:, ik, :, ikpq, :], optimize=True))
                        vbias_batch_plus_send[:, chol_idxs_chunk, iq] += .5j * (
                                xp.sqrt(2) * xp.einsum("pgi, aip -> ag", trial._rcholbara_chunk[iq, ik], Ghalfa[:, ikpq, :, ik, :], optimize=True))
